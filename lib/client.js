...
`;
		...
		function renderSentAnnotations(root, parsed) {
			...
			if (parsed.start > 0) fragment.appendChild(document.createTextNode(value.slice(0, parsed.start)));
			fragment.appendChild(makeSentAnnotationPill(parsed.items));
			if (parsed.end < value.length) fragment.appendChild(document.createTextNode(value.slice(parsed.end)));
		...
		function installSentAnnotationRenderer() {
			...
			ensureSentAnnotationStyle();
			const scan = () => {
				const candidates = Array.from(document.querySelectorAll("body *:not(script):not(style)"));
		...
		function addAnnotation(sessionId, text) {
			...
			const end = existing ? existing.offset + 1 : start;
		...
		function AnnotationPill(props) {
			...
			const removeAll = (event) => {
				...
				const afterPlaceholder = occurrence.offset + 1;
				const removeEnd = draft.slice(afterPlaceholder).trim() === "" ? draft.length : afterPlaceholder;
