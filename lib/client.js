...
`;
		...
		function makeSentAnnotationPill(items) {
			...
			button.setAttribute("aria-label", `${items.length} 条注释`);
			button.title = items.map((text, index) => `${index + 1}、${text}`).join("\n\n");
			button.textContent = `▤ ${items.length} 条注释`;
		...
		function installSentAnnotationRenderer() {
			...
			const scan = (roots) => {
				...
				candidates.forEach((element) => {
					...
					if (element.closest(`[${SENT_ANNOTATION_ATTR}]`) || element.querySelector(`[${SENT_ANNOTATION_ATTR}]`)) return;
